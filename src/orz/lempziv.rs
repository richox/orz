...
const LZ_MATCH_INDEX_ENCODING_ARRAY: [(u8, u8, u8); 4096] =
    include!("constants/LZ_MATCH_INDEX_ENCODING_ARRAY.txt");
const LZ_MATCH_INDEX_ID_BASE_ARRAY: [u16; 32] =
    include!("constants/LZ_MATCH_INDEX_ID_BASE_ARRAY.txt");
const LZ_MATCH_INDEX_BITS_LEN_ARRAY: [u8; 32] =
    include!("constants/LZ_MATCH_INDEX_BITS_LEN_ARRAY.txt");
...
impl LZEncoder {
    ...
    pub unsafe fn encode(
        ...
        while spos < sbuf.len() && match_items.len() < match_items.capacity() {
            ...
            match match_result {
                MatchResult::Match {
                    ...
                    if has_lazy_match {
                        match_items.push(MatchItem::Literal {
                            symbol: mtf.encode(*sbuf.get_unchecked(spos)),
                        });
                        spos += 1;
                    } else {
            ...
            match match_items.get_unchecked(match_items.len() - 1) {
                ...
                &MatchItem::Match {
                    ...
                    let (roid, _, _) =
                        *LZ_MATCH_INDEX_ENCODING_ARRAY.get_unchecked(reduced_offset as usize);
        ...
        [
            huff_encoder1.get_symbol_bits_lens(),
            huff_encoder2.get_symbol_bits_lens(),
        ].iter()
        ...
        for match_item in match_items.iter() {
            match match_item {
                ...
                &MatchItem::Match {
                    ...
                    let (roid, roid_rest_bits_len, roid_rest_bits) =
                        *LZ_MATCH_INDEX_ENCODING_ARRAY.get_unchecked(reduced_offset as usize);
...
impl LZDecoder {
    ...
    pub unsafe fn decode(
        ...
        let match_items_len = (tbuf[tpos + 0] as usize) << 0 | (tbuf[tpos + 1] as usize) << 8
        ...
        huff_symbol_bits_lenses
            .iter_mut()
            .for_each(|huff_symbol_bits_lens| {
                for i in 0..huff_symbol_bits_lens.len() / 2 {
                    *huff_symbol_bits_lens.get_unchecked_mut(i * 2 + 0) =
                        tbuf.get_unchecked(tpos + i) / 16;
                    *huff_symbol_bits_lens.get_unchecked_mut(i * 2 + 1) =
                        tbuf.get_unchecked(tpos + i) % 16;
                }
                tpos += huff_symbol_bits_lens.len() / 2;
            });
        ...
        for _ in 0..match_items_len {
            ...
            match huff_decoder1.decode_from_bits(bits) {
                ...
                match_len_x
                    if (LZ_MATCH_MIN_LEN as u16..LZ_MATCH_MAX_LEN as u16 + 1)
                        .contains(match_len_x - 256) =>
                {
                    ...
                    let reduced_offset =
                        reduced_offset_base + bits.get(reduced_offset_bits_len) as u16;
