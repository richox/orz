...
pub fn encode(source: &mut std::io::Read, target: &mut std::io::Write, params: &Params) -> std::io::Result<Statistics> {
    let time_start = std::time::SystemTime::now();
    let mut sbvec = vec![0u8; lempziv::LZ_BLOCK_SIZE];
    let mut tbvec = vec![0u8; lempziv::LZ_CHUNK_TARGET_SIZE];
    let mut lzenc = lempziv::Encoder::new();
    ...
    loop {
        ...
        while spos < sbvec_read_size {
            let (s, t) = unsafe {
                lzenc.encode(params, &sbvec[ .. sbvec_read_size], &mut tbvec, spos)
            };
            target.write_all(&[
                (t >>  0) as u8,
                (t >>  8) as u8,
...
pub fn decode(
    ...
    let time_start = std::time::SystemTime::now();
    let mut sbvec = vec![0u8; lempziv::LZ_BLOCK_SIZE + 512 /* as sentinel */];
    let mut tbvec = vec![0u8; lempziv::LZ_CHUNK_TARGET_SIZE];
    let mut lzdec = lempziv::Decoder::new();
    let mut statistics = Statistics {
        source_size: 0,
        target_size: 0,
    };
    ...
    loop {
        ...
        if spos == lempziv::LZ_BLOCK_SIZE || eof {
            statistics.source_size += spos as u64;
            statistics.target_size += tpos as u64;
            let time_end = std::time::SystemTime::now();
            let duration = time_end.duration_since(time_start).unwrap();
