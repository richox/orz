...
pub fn generate() {
    ...
    write!(fvalue, "[{}]", [
           ...
           256,
    ].iter().map(ToString::to_string).collect::<Vec<_>>().join(",")).unwrap();
    ...
    for i in 0 .. 257 {
        match i {
            256 => write!(fnext, "{},", 64).unwrap(),
            _   => write!(fnext, "{},", (i as f64 * 0.999999).powf(1.0 - 0.1 * i as f64 / 256.0).trunc()).unwrap(),
        }
    }
    write!(fnext, "]").unwrap();
}
